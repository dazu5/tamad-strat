...
This file is the process reference for the Tamad-Strat project. Strategy rules live in `STRATEGY_SPEC.md`.
...
- **Ablation (the core rebuild tool):** every rule is a pill. Enumerate on/off (or multi-setting) combinations of ALL rules — the Set-and-Forget study ran 200 configurations — and rank by **profit factor** (quality metric; trade count = quantity, return = consequence). The winner there had *everything off*: complexity was the disease.
...
- Exits deserve their own exploration phase (12 structures tested in the TJR rebuild; trailing 1R steps beat partials there). Exits matter as much as entries.
...
- **Equity-curve pain:** returns by year, flat/negative stretches, regime shifts (Set-and-Forget's edge concentrated post-2020). Can the trader psychologically survive the path?
